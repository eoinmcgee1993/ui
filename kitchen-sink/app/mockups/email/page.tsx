    ...
    <div style={{ display: "grid", gridTemplateColumns: "200px 360px 1fr", height: "calc(100vh - 52px)", background: "var(--bg-base)", color: "var(--text-primary)", fontFamily: "var(--font-ui)" }}>
      ...
      <aside style={{ borderRight: "1px solid var(--border-default)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: 12, borderTop: "1px solid var(--border-default)", display: "flex", alignItems: "center", gap: 8 }}>
          <div className="atlas-avatar">AD</div>
          <div style={{ flex: 1, fontSize: 12 }}>
            <div style={{ fontWeight: 500 }}>Adib</div>
            <div style={{ color: "var(--text-tertiary)" }}>admin@antarys.ai</div>
          </div>
          <button className="atlas-btn atlas-btn-icon" aria-label="Settings">
      ...
      <section style={{ borderRight: "1px solid var(--border-default)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ height: 56, boxSizing: "border-box", padding: "0 14px", borderBottom: "1px solid var(--border-default)", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div className="atlas-segmented">
            ...
            <button className="atlas-segmented-item">Unread</button>
          </div>
        </div>
        <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border-subtle)" }}>
          <input className="atlas-input" placeholder="Search mail…" style={{ width: "100%", height: 30, fontSize: 12 }} />
        </div>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {EMAILS.map((e) => (
            ...
            >
              {e.unread ? <span className="atlas-dot atlas-dot-info" style={{ marginTop: 7, flex: "none" }} /> : <span style={{ width: 6, flex: "none" }} />}
      ...
      <section style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ height: 56, boxSizing: "border-box", padding: "0 20px", borderBottom: "1px solid var(--border-default)", display: "flex", alignItems: "center", gap: 6 }}>
          <button className="atlas-btn atlas-btn-icon" aria-label="Back"><Icon paths="M19 12H5M12 19l-7-7 7-7" size={14} /></button>
          <button className="atlas-btn atlas-btn-icon" aria-label="Archive"><Icon paths="M21 8v13H3V8|M1 3h22v5H1zM10 12h4" size={14} /></button>
          <button className="atlas-btn atlas-btn-icon" aria-label="Spam"><Icon paths="circle:12,12,10|M12 8v4|M12 16h.01" size={14} /></button>
          <button className="atlas-btn atlas-btn-icon" aria-label="Trash"><Icon paths="polyline:3 6 5 6 21 6|M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6" size={14} /></button>
          <span style={{ width: 1, height: 16, background: "var(--border-default)", margin: "0 4px" }} />
          <button className="atlas-btn atlas-btn-icon" aria-label="Star"><Icon paths="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" size={14} /></button>
          <button className="atlas-btn atlas-btn-icon" aria-label="Label"><Icon paths="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z|M7 7h.01" size={14} /></button>
          <div style={{ flex: 1 }} />
          <span className="atlas-badge">{selected.label.toUpperCase()}</span>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 32px" }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div className="atlas-avatar">{selected.from.slice(0, 2).toUpperCase()}</div>
              <div style={{ flex: 1 }}>
              ...
              <button className="atlas-btn-pill atlas-btn-pill-secondary atlas-btn-pill-sm">Reply</button>
              <button className="atlas-btn-pill atlas-btn-pill-ghost atlas-btn-pill-sm">Forward</button>
            </div>
          </div>
          ...
          <div style={{ borderTop: "1px solid var(--border-subtle)", paddingTop: 20, fontSize: 14, lineHeight: 1.7, color: "var(--text-secondary)", maxWidth: 720 }}>
            ...
            <p style={{ margin: "0 0 12px" }}>
              Your production deployment of <code style={{ fontFamily: "var(--font-mono)", fontSize: 13, background: "var(--bg-input)", padding: "1px 6px", borderRadius: 3, border: "1px solid var(--border-default)", color: "var(--text-primary)" }}>atlas-edge</code> finished successfully. The build took <strong>1m 24s</strong> and passed all 247 tests.
            </p>
            <p style={{ margin: "0 0 16px" }}>Highlights from this deploy:</p>
            <ul style={{ paddingLeft: 20, margin: "0 0 16px", color: "var(--text-secondary)" }}>
              <li>Cache invalidation now supports tag-based clearing</li>
              <li>P95 latency dropped from <span style={{ fontFamily: "var(--font-mono)" }}>54ms</span> to <span style={{ fontFamily: "var(--font-mono)" }}>42ms</span></li>
              <li>Removed legacy Redis fallback — saving ~$180/mo</li>
            </ul>

            <div className="atlas-card" style={{ marginTop: 20 }}>
              <div className="atlas-card-header">
                <span className="atlas-dot atlas-dot-success" />
                <span className="atlas-panel-title">Deploy summary</span>
              </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
              <button className="atlas-btn-pill atlas-btn-pill-primary">View deploy →</button>
              <button className="atlas-btn-pill atlas-btn-pill-secondary">Roll back</button>
            </div>
        ...
        <div style={{ borderTop: "1px solid var(--border-default)", padding: 16, background: "var(--bg-raised)" }}>
          <div style={{ display: "flex", gap: 6, alignItems: "flex-start", padding: "8px 10px", background: "var(--bg-base)", border: "1px solid var(--border-default)", borderRadius: 8 }}>
            <div className="atlas-avatar" style={{ marginTop: 2 }}>AD</div>
            <input className="atlas-input" placeholder={`Reply to ${selected.from}…`} style={{ flex: 1, background: "transparent", border: 0, padding: "4px 0", height: 28, fontSize: 13 }} readOnly />
            <button className="atlas-btn atlas-btn-icon" aria-label="Attach"><Icon paths="M21.44 11.05 12.25 20.24a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" size={14} /></button>
