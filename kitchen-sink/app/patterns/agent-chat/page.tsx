  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", height: 620, background: "var(--bg-base)", color: "var(--text-primary)" }}>
      ...
      <main style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div className="atlas-titlebar" style={{ borderBottom: "1px solid var(--border-default)" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <span className="atlas-badge atlas-badge-info">claude-opus-4.7</span>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
            <div style={{ maxWidth: "70%", padding: "10px 14px", background: "var(--bg-raised)", border: "1px solid var(--border-default)", borderRadius: 10, fontSize: 13, lineHeight: 1.55 }}>
              The deploy to prod just failed — can you read the logs and tell me what broke?
            </div>
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <div className="atlas-avatar" style={{ background: "var(--accent-highlight-muted)", color: "var(--accent-highlight)", width: 22, height: 22, fontSize: 10 }}>A</div>
              <span style={{ fontSize: 11, color: "var(--text-tertiary)", fontFamily: "var(--font-mono)", letterSpacing: "0.05em" }}>ATLAS · 12:42:08</span>
            </div>

            <div style={{ fontSize: 13, lineHeight: 1.6, color: "var(--text-secondary)", marginBottom: 12 }}>
              Reading the deployment logs now.
            </div>
            ...
            <div className="atlas-card" style={{ marginBottom: 12 }}>
              <div className="atlas-card-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span className="atlas-panel-title">tool_call · fetch_deploy_logs</span>
                </span>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-tertiary)" }}>0.8s</span>
              </div>
              <div className="atlas-card-body" style={{ padding: 10, fontFamily: "var(--font-mono)", fontSize: 11, background: "var(--bg-input)", color: "var(--text-secondary)" }}>
                {"{ deploy_id: \"a9f3b1\", env: \"prod\", lines: 2048 }"}
              </div>
            </div>

            <div style={{ fontSize: 13, lineHeight: 1.6, color: "var(--text-secondary)", marginBottom: 12 }}>
              The failure is at <code style={{ fontFamily: "var(--font-mono)", fontSize: 12, background: "var(--bg-input)", padding: "1px 6px", borderRadius: 3, color: "var(--text-primary)", border: "1px solid var(--border-default)" }}>build.sh:42</code> — a missing env var <code style={{ fontFamily: "var(--font-mono)", fontSize: 12, background: "var(--bg-input)", padding: "1px 6px", borderRadius: 3, color: "var(--text-primary)", border: "1px solid var(--border-default)" }}>DATABASE_URL</code>.
              According to <a href="#" className="atlas-link">the platform dashboard <sup style={{ color: "var(--accent-highlight)" }}>[1]</sup></a>, it was removed 4 minutes ago during the staging cleanup.
            </div>

            <div className="atlas-alert atlas-alert-warning" style={{ marginBottom: 12 }}>
              ...
              <div className="atlas-alert-title">Env var was shared</div>
              <div className="atlas-alert-desc">
                <code style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>DATABASE_URL</code> is used by 3 other services. Restoring will re-enable <code style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>payment-api</code> and <code style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>inbox-worker</code>.
              </div>
            </div>

            <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
              <button className="atlas-btn atlas-btn-primary">Restore env var</button>
              <button className="atlas-btn atlas-btn-secondary">Show full stacktrace</button>
          ...
          <div style={{ marginTop: 32, paddingTop: 12, borderTop: "1px dashed var(--border-default)", display: "flex", gap: 8, fontSize: 11, color: "var(--text-tertiary)" }}>
            ...
            <a href="#" className="atlas-link" style={{ fontSize: 11 }}>platform.example.com/env-vars</a>
          </div>
        </div>
...
export default function AgentChatPatternPage() {
  return (
    <PageBody
      inspector={
        <Inspector
          anatomy="260px conversation rail · main thread · tool-call cards · citation footer · composer at bottom."
          tokens={[".atlas-card", ".atlas-alert", ".atlas-badge", ".atlas-dot.is-pulse", ".atlas-btn-pill"]}
          dos={[
            "Render tool calls as cards — they're structured, not conversational",
            "Show the model name as a badge, not buried in metadata",
            "Cite sources inline with superscript + footer",
            "Pulse the status dot while the agent is running",
          ]}
          donts={[
            "Don't render raw JSON without the card wrapper",
            "Don't use blue anywhere except citations, focus, and the model badge",
            "Don't animate message arrival — instant rendering feels snappier",
          ]}
        />
      }
    >
      <PageHeader
        eyebrow="Patterns · shadcn for agent UI"
        title="Agent chat"
        subtitle="The canonical agent console: conversation history rail, user + assistant turns, tool-call cards, inline citations, composer with token meter. This is Atlas's pitch — the same dense design language that powers dashboards now shapes every agent product."
      />
      <Section title="Live preview" desc="Rendered inside .atlas-device-desktop so it reads as a shipped product, not docs.">
        <div style={{ display: "flex", justifyContent: "center", padding: "24px 0" }}>
          <DeviceFrame variant="desktop" url="ui.pacifio.dev/chat">
            <AgentChatMock />
          </DeviceFrame>
        </div>
      </Section>
