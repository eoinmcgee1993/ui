...
function EmailApp() {
  ...
  const emails = [
    { from: "Vercel", subject: "Your deployment completed", preview: "atlas-edge finished building in 1m 24s …", time: "12:42", unread: true },
    { from: "GitHub", subject: "PR #217 needs review", preview: "feat(cache): add tag-based invalidation …", time: "11:08", unread: true },
    { from: "Linear", subject: "3 new issues assigned", preview: "ATLAS-42 · AMOLED light-mode shadows too strong", time: "09:50", unread: true },
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px 10px", borderBottom: "1px solid var(--border-default)", flex: "none" }}>
        ...
        <button className="atlas-btn atlas-btn-icon" aria-label="Compose">
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {tab === "inbox" &&
          emails.map((e) => (
            <div key={e.subject} style={{ padding: "12px 16px", borderBottom: "1px solid var(--border-subtle)", display: "flex", gap: 12, alignItems: "flex-start" }}>
              {e.unread && <span className="atlas-dot atlas-dot-info" style={{ marginTop: 6, flex: "none" }} />}
        ...
        {tab === "starred" && (
          <div className="atlas-empty" style={{ padding: 48 }}>
            <div className="atlas-empty-icon">
              <Icon paths={ICONS.star} />
            </div>
            <div className="atlas-empty-title">Nothing starred yet</div>
            <div className="atlas-empty-desc">Tap the star on any email to pin it here.</div>
          </div>
        )}
        {tab === "sent" && (
        ...
        {tab === "me" && (
          <div style={{ padding: "24px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div className="atlas-avatar" style={{ width: 44, height: 44, fontSize: 16 }}>AD</div>
              <div>
      ...
      <nav className="atlas-tabbar-bottom">
        {tabs.map((t) => (
          <button
            key={t.id}
            className={`atlas-tabbar-bottom-item${tab === t.id ? " is-active" : ""}`}
            onClick={() => setTab(t.id)}
          >
...
function MusicApp() {
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px 10px", borderBottom: "1px solid var(--border-default)", flex: "none" }}>
        <div style={{ fontSize: 18, fontWeight: 600 }}>{tabs.find((t) => t.id === tab)?.label}</div>
        <button className="atlas-btn atlas-btn-icon" aria-label="Profile">
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <div className="atlas-filmstrip" style={{ width: 64, height: 64, borderRadius: 6, background: "linear-gradient(135deg,#262626 0%,#0a0a0a 100%)", flex: "none" }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <div className="atlas-progress" style={{ height: 3 }}>
                      <div className="atlas-progress-bar" style={{ width: "34%" }} />
                    </div>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 14 }}>
                <button className="atlas-btn atlas-btn-icon" aria-label="Previous">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M19 20L9 12l10-8v16zM5 19V5h2v14H5z" /></svg>
                </button>
                <button className="atlas-btn atlas-btn-primary" style={{ borderRadius: 9999, width: 40, height: 40, padding: 0 }} aria-label="Pause">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M6 4h4v16H6zM14 4h4v16h-4z" /></svg>
                </button>
                <button className="atlas-btn atlas-btn-icon" aria-label="Next">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {playlists.map((p) => (
                  <div key={p.title} style={{ padding: 10, border: "1px solid var(--border-default)", borderRadius: 6, background: "var(--bg-raised)" }}>
                    <div className="atlas-filmstrip" style={{ aspectRatio: "1 / 1", borderRadius: 4, marginBottom: 8, background: "linear-gradient(135deg,#262626 0%,#0a0a0a 100%)" }} />
        ...
        {tab === "search" && (
          <div style={{ padding: 16 }}>
            <input className="atlas-input" placeholder="Artists, songs, albums…" style={{ width: "100%", height: 36, fontSize: 13 }} />
          ...
          <div style={{ padding: 16 }}>
            {playlists.map((p) => (
              <div key={p.title} style={{ padding: "12px 0", borderBottom: "1px solid var(--border-subtle)", display: "flex", gap: 12, alignItems: "center" }}>
                <div className="atlas-filmstrip" style={{ width: 44, height: 44, borderRadius: 4, background: "linear-gradient(135deg,#262626,#0a0a0a)", flex: "none" }} />
                <div style={{ flex: 1 }}>
        ...
        {tab === "liked" && (
          <div className="atlas-empty" style={{ padding: 48 }}>
            <div className="atlas-empty-icon"><Icon paths={ICONS.heart} /></div>
            <div className="atlas-empty-title">No likes yet</div>
            <div className="atlas-empty-desc">Hit the heart on any song to save it.</div>
          </div>
        )}
      </div>
...
const snippet = `<nav class="atlas-tabbar-bottom">
    ...
    <PageBody
      inspector={
        <Inspector
          ...
          donts={["Don't use on desktop — use .atlas-sidebar nav", "Don't hide the tab bar on scroll"]}
        />
      }
    >
      <PageHeader
        eyebrow=".atlas-tabbar-bottom"
        title="Mobile bottom tab bar"
        subtitle="Native-style bottom navigation. 5 tabs, 20px icons, 10px labels. Active tab brightens + icon stroke thickens."
        meta={[".atlas-tabbar-bottom", ".atlas-tabbar-bottom-item", ".is-active"]}
      />

      <Section title="Standalone" desc="The bar itself, rendered at real size without a device frame — so you can see the visual detail.">
        <div className="ks-preview ks-preview-block" style={{ padding: 0 }}>
          <div style={{ width: "100%", maxWidth: 440, margin: "0 auto" }}>
            <nav className="atlas-tabbar-bottom" style={{ borderTop: "1px solid var(--border-default)", borderBottom: "1px solid var(--border-default)" }}>
              <button className="atlas-tabbar-bottom-item is-active">
                <Icon paths={ICONS.home} stroke={2} />
                <span>Home</span>
              </button>
              <button className="atlas-tabbar-bottom-item">
      ...
      <Section title="Live — Email app" desc="Five-tab email client: Inbox / Starred / Sent / Archive / Me. Tap the tabs to switch screens. Uses atlas-tabbar-bottom with a compose icon button in the header.">
      ...
      <Section title="Live — Music app" desc="Four-tab music player: Listen / Search / Library / Liked. Now-playing row with progress bar + transport controls demonstrates how the tab bar coexists with rich content.">
        <div style={{ display: "flex", justifyContent: "center", padding: "24px 0" }}>
          <div style={{ transform: "scale(0.72)", transformOrigin: "top center", marginBottom: -260 }}>
            <DeviceFrame variant="mobile">
              <MusicApp />
