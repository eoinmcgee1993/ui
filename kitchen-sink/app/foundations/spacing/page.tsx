...
export default function SpacingPage() {
  return (
    <PageBody
      inspector={
        <Inspector
          anatomy="Strict 4px grid. 2px and 6px exist for ultra-compact inline spacing only."
          dos={["Stay on the 4px grid — multiples of 4", "Use 12 / 16 / 24 for the vast majority of gaps"]}
          donts={["Don't invent values (e.g. 10px, 18px) — pick from the scale", "Don't mix 6 and 8 in the same layout"]}
        />
      }
    >
      <PageHeader
        eyebrow="Foundations"
        title="Spacing"
        subtitle="A strict 4px grid. Twelve tokens cover every gap, margin, and padding need. Tight by default — Atlas is a devtool, not a consumer app."
      />
      <Section title="Scale">
        <ScaleGrid items={scale} />
      </Section>
      <Section title="Density guidance" desc="Recommended spacing for common UI density decisions.">
